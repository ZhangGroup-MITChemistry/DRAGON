...
class ProcessCTCFSites():
	...
	#
		posSta = chr_region[str(chrId)][0]
		posEnd = chr_region[str(chrId)][1]

		outfile = './%s/%s_chr%d_ctcf_position_From%dMbTo%dMb.txt'%(celltype,celltype,chrId,posSta,posEnd)
		final_ctcf_states = []
		...
		if not os.path.exists(outfile):
			...
			np_path = '%s/../../../../processEpigenomicsData/ctcfBindingSites/raw.narrowPeak'%(glb_path)
			ctcf_peaks_all = np.loadtxt('%s/%s/ctcf/chip-seq_peak_%d.txt'%(np_path,celltype,chrId))
			rad21_peaks_all = np.loadtxt('%s/%s/rad21/chip-seq_peak_%d.txt'%(np_path,celltype,chrId))

			ctcf_peaks = [peak for peak in ctcf_peaks_all if peak[0] >= posSta and peak[1] <= posEnd]
			rad21_peaks = [peak for peak in rad21_peaks_all if peak[0] >= posSta and peak[1] <= posEnd]
			...
			try:
				...
				print('   > The selected region is beyond the length of chromosome %d.\n'%chrId)
				sys.exit()
			...
			flag = 0
			print('''   > CTCF-binding sites of %s, chrom%d exists, ready to use!'''\
																%(celltype,chrId))
		return final_ctcf_states,flag
