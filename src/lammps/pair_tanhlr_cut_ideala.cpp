...
#include "stdlib.h"
#include "string.h"
#include "pair_tanhlr_cut_ideala.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairTanhlrCutIdeala::PairTanhlrCutIdeala(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 0;
}
...
PairTanhlrCutIdeala::~PairTanhlrCutIdeala()
{
  if (allocated) {
...
void PairTanhlrCutIdeala::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if ( (rsq < cutsq[itype][jtype]) && (jmi<nideal) ){
        ...
        double ideal_alpha;
        ideal_alpha = ideal_potential[jmi][active[jglobal-1]-1][active[iglobal-1]-1];
...
void PairTanhlrCutIdeala::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairTanhlrCutIdeala::settings(int narg, char **arg)
{
...
// the second argument is the input file for ideal chromosome potential
  ...
  memory->create(ideal_potential,nideal, nChromatin, nChromatin, "pair:ideal");
  ...
  while(fgets(line,102400,fp)) {
    ...
    // The second site
    for (int ichr=0; ichr<nChromatin; ichr++) {
        for (int jchr=ichr; jchr<nChromatin; jchr++) {
            ptr = strtok(NULL," \t\n\r\f");
            if (!ptr) error->all(FLERR,"Incorrectly formatted ideal chromosome potential file");
            ideal_potential[idx][ichr][jchr] = force->numeric(FLERR, ptr) * 0.5;  // note that 0.5 is added here. This is because the contact function
            ...
            ideal_potential[idx][jchr][ichr] = ideal_potential[idx][ichr][jchr];
        }
    }
  }
  fclose(fp); 
...
void PairTanhlrCutIdeala::coeff(int narg, char **arg)
{
...
double PairTanhlrCutIdeala::init_one(int i, int j)
{
...
void PairTanhlrCutIdeala::write_restart(FILE *fp)
{
...
void PairTanhlrCutIdeala::read_restart(FILE *fp)
{
...
void PairTanhlrCutIdeala::write_restart_settings(FILE *fp)
{
...
void PairTanhlrCutIdeala::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
double PairTanhlrCutIdeala::single(int i, int j, int itype, int jtype, double rsq,
...
/* ---------------------------------------------------------------------- */
double PairTanhlrCutIdeala::memory_usage()
{
